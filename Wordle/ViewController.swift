...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        // STEP 1: Assigns Closure to `didSelectString`.property of `keyboardController`.
        keyboardController.didSelectString = {
            // STEP 2: Takes in a string
            selectedString in
            // STEP 3: If the string is equal to the `DELETE_KEY` constant, then call the `deleteLastCharacter` method of `boardController`.
            if selectedString == DELETE_KEY {
                self.boardController.deleteLastCharacter()
            //STEP 4: Else, it should use the `enter` method of `boardController` and pass in the selected string as the argument.
            } else {
